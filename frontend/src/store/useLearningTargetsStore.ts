...
interface LearningTargetsState {
  ...
  createTarget: (
    target: Omit<LearningTarget, "id" | "createdAt" | "updatedAt">,
  ) => Promise<void>;
...
export const useLearningTargetsStore = create<LearningTargetsState>(
  (set, get) => ({
    targets: [],
    isLoading: false,
    error: null,
    ...
    getTargetsByStatus: (status) => {
      ...
      if (selectedCourseId) {
        filteredTargets = filteredTargets.filter(
          (target) => target.courseId === selectedCourseId,
        );
      }
    ...
    setTemporaryTargetsFromQuiz: (questions) => {
      ...
      questions.forEach((question) => {
        ...
        const subTopicValue =
          question.subTopic || (question as any).subTopicName;
        if (subTopicValue) {
      ...
      const temporaryTargets: TemporaryLearningTarget[] = Array.from(
        uniqueSubTopics,
      ).map((subTopic) => ({
        topic:
          questions.find((q) => {
    ...
    updateTemporaryTargetScores: (quizResults) => {
      ...
      const updatedTargets = temporaryTargets.map((target) => {
        ...
        const subTopicQuestions =
          quizResults.questions?.filter((q: any) => {
