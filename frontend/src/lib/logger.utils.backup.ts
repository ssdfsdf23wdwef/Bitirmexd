...
export function setupLogger(options?: any): any {
  ...
  try {
    ...
    if (typeof window === 'undefined') {
      // Server-side: return a mock logger
      return {
    ...
    try {
      LoggerService = require('../services/logger.service').LoggerService;
    } catch (requireError) {
      // Fallback to ES module import
      const module = eval('import("../services/logger.service")');
      if (module && module.LoggerService) {
        LoggerService = module.LoggerService;
      } else {
        throw new Error('LoggerService not found in module');
      }
    }
    
    if (!LoggerService || typeof LoggerService.getInstance !== 'function') {
      throw new Error('LoggerService not properly exported or getInstance method missing');
    }
