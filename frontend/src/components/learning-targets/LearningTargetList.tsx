...
const LearningTargetList: React.FC<LearningTargetListProps> = ({
  courseId,
}) => {
  const { targets, isLoading, error, fetchTargets, setSelectedCourseId } =
  ...
  const groupedByStatus = {
    [LearningTargetStatus.NOT_STARTED]: filteredTargets.filter(
      (t) => t.status === LearningTargetStatus.NOT_STARTED,
    ),
    [LearningTargetStatus.IN_PROGRESS]: filteredTargets.filter(
      (t) => t.status === LearningTargetStatus.IN_PROGRESS,
    ),
    [LearningTargetStatus.COMPLETED]: filteredTargets.filter(
      (t) => t.status === LearningTargetStatus.COMPLETED,
    ),
  };
