...
const LearningTargetsPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<LearningTargetStatus | "all">(
    "all",
  );

  const { fetchTargets, targets, getTargetsByStatus } =
  ...
  const countByStatus = {
    [LearningTargetStatus.NOT_STARTED]: getTargetsByStatus(
      LearningTargetStatus.NOT_STARTED,
    ).length,
    [LearningTargetStatus.IN_PROGRESS]: getTargetsByStatus(
      LearningTargetStatus.IN_PROGRESS,
    ).length,
    [LearningTargetStatus.COMPLETED]: getTargetsByStatus(
  ...
  const handleTabChange = (
    ...
    newValue: LearningTargetStatus | "all",
  ) => {
      ...
      <Box mb={4}>
        ...
        {courseId && (
          <Typography variant="body1" color="text.secondary">
            Bu sayfada, seçili ders için öğrenme hedeflerinizi görüntüleyebilir,
            yeni hedefler ekleyebilir ve mevcut hedefleri yönetebilirsiniz.
          </Typography>
        )}
      </Box>
        ...
        <Box display="flex" justifyContent="space-around" alignItems="center">
          ...
          <Box textAlign="center" p={2}>
            ...
            <Typography variant="h3">
              {countByStatus[LearningTargetStatus.NOT_STARTED]}
            </Typography>
          </Box>
          <Divider orientation="vertical" flexItem />
          <Box textAlign="center" p={2}>
            ...
            <Typography variant="h3">
              {countByStatus[LearningTargetStatus.IN_PROGRESS]}
            </Typography>
          </Box>
          <Divider orientation="vertical" flexItem />
          <Box textAlign="center" p={2}>
            ...
            <Typography variant="h3">
              {countByStatus[LearningTargetStatus.COMPLETED]}
            </Typography>
          </Box>
        </Box>
      </Paper>
      ...
      {activeTab === "all" ? (
        ...
        <Box>
          <Typography variant="h6" sx={{ mb: 2 }}>
            {activeTab === LearningTargetStatus.NOT_STARTED &&
              "Başlanmamış Hedefler"}
            {activeTab === LearningTargetStatus.IN_PROGRESS &&
              "Devam Eden Hedefler"}
            {activeTab === LearningTargetStatus.COMPLETED &&
