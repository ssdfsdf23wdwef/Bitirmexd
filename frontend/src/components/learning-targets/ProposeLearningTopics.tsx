...
const getAiTopicSuggestions = async (
  courseId: string,
  context: string,
  ...
  try {
    ...
    throw new Error(
      "AI konu önerileri alınırken bir hata oluştu. Lütfen tekrar deneyin.",
    );
  }
};
...
const confirmTopics = async (
  ...
  try {
    console.log("Confirming topics via real backend API:", {
      courseId,
      selectedTopics,
    });

    // Call the new backend API using confirmProposedTopics
    const result = await learningTargetService.confirmProposedTopics(
      courseId,
      selectedTopics,
    );
...
const ProposeLearningTopics: React.FC<ProposeLearningTopicsProps> = ({
  ...
  const handlePropose = async () => {
    console.group(
      "🤖 [ProposeLearningTopics] handlePropose - AI konu önerisi başlatılıyor",
    );
    ...
    try {
      ...
      const topics = await getAiTopicSuggestions(
        courseId,
        contextText,
        existingTopics,
      );
      ...
      console.log("✅ AI önerileri başarıyla alındı:", {
        ...
        relevanceDistribution: topics.reduce(
          (acc, topic) => {
      ...
      setError(
        "Konu önerileri alınırken bir hata oluştu. Lütfen tekrar deneyin.",
      );
    } finally {
  ...
  const handleTopicToggle = (topic: ProposedTopic) => {
    console.group(
      "🔄 [ProposeLearningTopics] handleTopicToggle - Konu seçimi değiştiriliyor",
    );
    console.log("📋 Parametreler:", {
      ...
      isCurrentlySelected: selectedTopics.some(
  ...
  const handleConfirm = async () => {
    console.group(
      "✅ [ProposeLearningTopics] handleConfirm - Seçili konular onaylanıyor",
    );
  ...
  const handleClose = () => {
    console.group(
      "🚪 [ProposeLearningTopics] handleClose - Dialog kapatılıyor",
    );
    ...
    switch (step) {
        ...
        return (
          <>
            <Typography variant="body1" gutterBottom>
              Ders notlarınızı veya ilgili metni aşağıya yapıştırın. Yapay zeka,
              bu içeriği analiz ederek öğrenmeniz gereken yeni konuları
              önerecektir.
            </Typography>
              ...
              >
                ...
                <Box display="flex" flexWrap="wrap" gap={1}>
                  ...
                  {existingTopics.length > 5 && (
                    <Chip
                      label={`+${existingTopics.length - 5} daha`}
                      size="small"
                    />
                  )}
                </Box>
      ...
      case "review":
        return (
          <>
            <Typography variant="body1" gutterBottom>
              Yapay zeka, metin içinde aşağıdaki yeni konuları tespit etti.
              Öğrenme hedefi olarak eklemek istediklerinizi seçin.
            </Typography>
            ...
            {proposedTopics.length === 0 && (
              <Alert severity="info" sx={{ mt: 2 }}>
                Metinde öğrenmeniz gereken yeni konular tespit edilemedi. Farklı
                bir metin deneyebilir veya manuel olarak öğrenme hedefi
                ekleyebilirsiniz.
              </Alert>
            )}
          </>
        ...
        return (
          <Box textAlign="center" py={2}>
            ...
            <Typography variant="body1">
              {selectedTopics.length} yeni öğrenme hedefi oluşturuldu. Öğrenme
              hedefleri sayfasından bu konuları yönetebilirsiniz.
            </Typography>
          </Box>
        );
    }
