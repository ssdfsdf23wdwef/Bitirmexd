...
import { FiBook, FiSettings, FiFileText, FiTarget, FiList, FiCheckCircle, FiArrowRight } from "react-icons/fi";
...
const ExamCreationProgress: React.FC<ExamCreationProgressProps> = ({
  ...
  return (
    <div className={`flex w-full h-full min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      {/* Sidebar */}
      <div className={`w-80 flex flex-col border-r ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
        <div className="p-8 h-full flex flex-col">
          {/* Header */}
          <div className="mb-8">
            <div className={`inline-flex items-center justify-center w-12 h-12 rounded-lg mb-4 ${isDarkMode ? 'bg-blue-600' : 'bg-blue-500'}`}>
            ...
            <p className={`text-sm mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              {quizType === "personalized" ? "Kişiselleştirilmiş" : "Hızlı"} sınav oluşturma süreci
            </p>
          </div>
            ...
            {(quizType === "personalized" ? stepTitles.personalized : stepTitles.quick).map((title, index) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <FiArrowRight className={`w-4 h-4 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                  )}
                </div>
              );
            })}
        ...
        <div className="h-full p-8 flex flex-col">
          ...
          <div className="mb-8">
            <div className="flex items-center mb-4">
              <div className={`w-1 h-8 rounded-full mr-4 ${isDarkMode ? 'bg-blue-500' : 'bg-blue-500'}`} />
              <h1 className={`text-2xl font-semibold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                {currentStepTitle}
              </h1>
            </div>
          ...
          {/* Content Container */}
          <div className="flex-1 relative">
            <div className={`h-full rounded-lg p-6 border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              {children}
            </div>
          </div>
        </div>
