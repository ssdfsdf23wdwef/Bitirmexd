...
const LearningProgress: React.FC<LearningProgressProps> = ({
      ...
      >
        <div className="flex items-start justify-between">
          ...
          <div className={`p-3 rounded-xl ${
            ...
            {status === "mastered" ? (
              ...
              <FiTarget className={`text-2xl ${isDarkMode ? 'text-amber-400' : 'text-yellow-600'}`} />
            )}
          </div>
        </div>
        ...
        {data && (
          <div className="mt-6">
            ...
            <div className={`w-full rounded-full h-3 ${isDarkMode ? 'bg-slate-700' : 'bg-gray-200'} overflow-hidden`}>
              <div
            ...
            <div className={`flex justify-between mt-4 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
  ...
  return (
    <div className={`space-y-8 transition-all duration-500 ${isDarkMode ? 'bg-slate-900' : 'bg-gray-50'}`}>
      {/* Genel İlerleme */}
      <div className={`rounded-2xl shadow-xl backdrop-blur-lg p-8 transition-all duration-500 hover:shadow-2xl ${
        isDarkMode ? 'bg-slate-800/70 border border-slate-700/50' : 'bg-white/70 border border-gray-200/50'
      }`}>
        <h2 className={`text-2xl font-bold mb-6 flex items-center ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>
          <div className={`p-2 rounded-lg mr-3 ${isDarkMode ? 'bg-blue-500/20 text-blue-400 border border-blue-400/30' : 'bg-indigo-100 text-indigo-600'}`}>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-8">
          ...
          <div className="flex-1 space-y-4">
            <div className={`grid grid-cols-1 md:grid-cols-3 gap-4`}>
              <div className={`p-5 rounded-xl border transition-all duration-300 hover:shadow-lg ${
                isDarkMode ? 'bg-slate-700/50 border-slate-600/50' : 'bg-gray-50/70 border-gray-200'
              }`}>
              ...
              <div className={`p-5 rounded-xl border transition-all duration-300 hover:shadow-lg ${
                ...
                <div className={`text-2xl font-bold ${isDarkMode ? 'text-amber-400' : 'text-yellow-600'}`}>
              ...
              <div className={`p-5 rounded-xl border transition-all duration-300 hover:shadow-lg ${
                ...
                <div className={`text-2xl font-bold ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>
                  {(strongTopics?.length || 0) + Object.keys(weakTopics || {}).length}
                </div>
                <div className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  Toplam Konu
                </div>
            ...
            <div className={`p-5 rounded-xl border ${
              masteryPercentage >= 80
                ? isDarkMode ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-green-50 border-green-200'
                : masteryPercentage >= 60
                  ? isDarkMode ? 'bg-amber-500/10 border-amber-500/30' : 'bg-yellow-50 border-yellow-200'
      ...
      {strongTopics.length > 0 && (
        ...
        >
          <h2 className={`text-xl font-semibold mb-6 flex items-center ${isDarkMode ? 'text-green-400' : 'text-green-600'}`}>
            <div className={`p-2 rounded-lg mr-3 ${isDarkMode ? 'bg-green-500/20 text-green-400' : 'bg-green-100 text-green-600'}`}>
      ...
      {Object.values(weakTopics).filter((topic) => topic.status === "active").length > 0 && (
        ...
        >
          <h2 className={`text-xl font-semibold mb-6 flex items-center ${isDarkMode ? 'text-yellow-400' : 'text-amber-600'}`}>
            <div className={`p-2 rounded-lg mr-3 ${isDarkMode ? 'bg-yellow-500/20 text-yellow-400' : 'bg-yellow-100 text-amber-600'}`}>
