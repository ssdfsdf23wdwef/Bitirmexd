import { ApiFailedQuestion, ApiQuiz } from "./adapter.service";
...
class QuizApiService {
  ...
  public storeQuizInStorage(quiz: Quiz): void {
    if (!quiz || !quiz.id) {
      console.error(
        "[QuizApiService.storeQuizInStorage] Geçersiz quiz nesnesi. ID bulunamadı.",
      );
      return;
    }
    ...
    try {
        ...
        enhancedQuiz.questions = enhancedQuiz.questions.map((q) => {
          ...
          if (
            ...
            if (index !== -1) {
              ...
              console.log(
                `[QuizApiService.storeQuizInStorage] ✅ correctAnswerId eklendi: ${question.id}, index=${index}`,
              );
            } else {
              console.warn(
                `[QuizApiService.storeQuizInStorage] ⚠️ correctAnswer seçenekler arasında bulunamadı: ${question.id}`,
              );
      ...
      console.log(
        `[QuizApiService.storeQuizInStorage] ✅ Quiz başarıyla localStorage'a kaydedildi: ID=${quiz.id}`,
      );
    } catch (error) {
      console.error(
        "[QuizApiService.storeQuizInStorage] Quiz localStorage'a kaydedilemedi:",
        error,
      );
    }
  }
  ...
  public getQuizFromLocalStorage(quizId: string): Quiz | null {
    ...
    try {
      ...
      console.error(
        `[QuizApiService.getQuizFromLocalStorage] Quiz okuma hatası: ${error}`,
      );
      return null;
    }
  }
  ...
  private getResponseData(response: unknown): unknown {
    try {
      ...
      if (typeof response === "object" && response !== null) {
        ...
        console.log(
          "[QuizApiService.getResponseData] Yanıt nesne anahtarları:",
          keys.join(", "),
        );
  ...
  private isObject(value: unknown): value is Record<string, unknown> {
    ...
    console.log(
      `[QuizApiService.isObject] Değer tipi: ${typeof value}, null mu: ${value === null}, dizi mi: ${Array.isArray(value)}, sonuç: ${result}`,
    );
    return result;
  }
  ...
  private isApiResponse(response: unknown): response is BaseApiResponse {
    try {
      ...
      const hasStatusAndData =
        isObject && "status" in response && "data" in response;

      console.log(
        `[QuizApiService.isApiResponse] isObject: ${isObject}, hasStatusAndData: ${hasStatusAndData}`,
      );

      if (hasStatusAndData) {
        console.log(
          `[QuizApiService.isApiResponse] status tipi: ${typeof (response as Record<string, unknown>).status}, data tipi: ${typeof (response as Record<string, unknown>).data}`,
        );
      }
  ...
  private isValidApiQuiz(data: unknown): data is ApiQuiz {
    try {
      ...
      if (!this.isObject(data)) {
        console.log(
          `[QuizApiService.isValidApiQuiz] Object değil, false dönülüyor`,
        );
        return false;
      }
      ...
      console.log(
        `[QuizApiService.isValidApiQuiz] id var mı: ${hasId}, id: ${q.id}, questions dizisi mi: ${hasQuestions}, questions.length: ${hasQuestions ? q.questions.length : "N/A"}`,
      );
  ...
  private createFallbackQuestion(index: number): Question {
    console.log(
      `[QuizApiService.createFallbackQuestion] Varsayılan soru oluşturuluyor. Index: ${index}`,
    );
    const question = {
      id: `fallback_q_${index}_${Date.now()}`,
      questionText: `Varsayılan Soru ${index + 1}`,
      options: [
        "Varsayılan Seçenek A",
        "Varsayılan Seçenek B",
        "Varsayılan Seçenek C",
        "Varsayılan Seçenek D",
      ],
    ...
    console.log(
      `[QuizApiService.createFallbackQuestion] Varsayılan soru oluşturuldu: ${question.id}`,
    );
    return question;
  }
  ...
  private createFallbackQuiz(
    ...
    console.log(
      `[QuizApiService.createFallbackQuiz] Varsayılan sınav oluşturuluyor. Prefix: ${idPrefix}, options:`,
      options,
    );
    ...
    console.log(
      `[QuizApiService.createFallbackQuiz] ID: ${fallbackId}, soru sayısı: ${questionCount}`,
    );
    ...
    const quiz = {
      ...
      description:
        options?.description ||
        "Bu sınav, API yanıtı işlenirken bir sorun oluştuğu için varsayılan olarak oluşturulmuştur.",
      questions: Array.from({ length: questionCount }, (_, i) =>
        this.createFallbackQuestion(i),
      ),
    ...
    console.log(
      `[QuizApiService.createFallbackQuiz] Varsayılan sınav oluşturuldu: ${quiz.id}, sorular:`,
  ...
  async getQuizById(id: string): Promise<Quiz | null> {
    try {
      ...
      console.log(
        "[DEBUG] quiz.service.ts - getQuizById - Ham apiQuiz:",
        JSON.stringify(apiQuiz, null, 2),
      );
  ...
  async getQuizAnalysis(quizId: string): Promise<QuizAnalysisResponse> {
    try {
      return await this.apiService.safeRequest(
        ...
        async () => {
          try {
            ...
            const result = await this.apiService.get<QuizAnalysisResponse>(
              `quizzes/analysis/${quizId}`,
            );
            console.log(
              "[QuizApiService.getQuizAnalysis] API yanıtı (quizzes/analysis):",
              result,
            );
            ...
            if (axios.isAxiosError(err) && err.response?.status === 404) {
              console.log(
                "[QuizApiService.getQuizAnalysis] Alternatif endpoint deneniyor (quiz-analysis)",
              );
              // Corrected fallback endpoint: remove leading slash
              const result = await this.apiService.get<QuizAnalysisResponse>(
                `quiz-analysis/${quizId}`,
              );
              console.log(
                "[QuizApiService.getQuizAnalysis] API yanıtı (quiz-analysis):",
                result,
              );
              return result as QuizAnalysisResponse;
            }
      ...
      console.error(
        "[QuizApiService.getQuizAnalysis] Beklenmeyen hata:",
        error,
      );
  ...
  async getFailedQuestions(courseId?: string) {
    try {
      const params: Record<string, string> = {};
      if (courseId) params.courseId = courseId;
      const response = await apiService.get<ApiFailedQuestion[]>(
        this.failedQuestionsPath,
        params,
      );
      const failedQuestions = this.getResponseData(
        response,
      ) as ApiFailedQuestion[];

      return failedQuestions;
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      const apiError = ErrorService.createApiError(
  ...
  private mapResponseToQuiz(response: QuizResponseDto): Quiz {
    try {
      console.log(
        "[QuizApiService.mapResponseToQuiz] Dönüştürülecek yanıt:",
        response,
      );

      // Input detaylarını kontrol et
      console.log(
        `[QuizApiService.mapResponseToQuiz] id varlığı: ${!!response.id}, tipi: ${typeof response.id}`,
      );
      console.log(
        `[QuizApiService.mapResponseToQuiz] title varlığı: ${!!response.title}, tipi: ${typeof response.title}`,
      );
      console.log(
        `[QuizApiService.mapResponseToQuiz] quizType varlığı: ${!!response.quizType}, tipi: ${typeof response.quizType}`,
      );
      console.log(
        `[QuizApiService.mapResponseToQuiz] questions varlığı: ${!!response.questions}, dizi mi: ${Array.isArray(response.questions)}, uzunluk: ${Array.isArray(response.questions) ? response.questions.length : "N/A"}`,
      );
      console.log(
        `[QuizApiService.mapResponseToQuiz] dates: createdAt=${response.createdAt}, updatedAt=${response.updatedAt}`,
      );
      ...
      const result: Quiz = {
        ...
        preferences: {
          questionCount: response.questions?.length || 10,
          difficulty: "medium",
      ...
      console.log(
        "[QuizApiService.mapResponseToQuiz] Dönüştürülen quiz nesnesi:",
        result,
      );
      console.log(
        `[QuizApiService.mapResponseToQuiz] Quiz oluşturma başarılı. ID: ${result.id}, soru sayısı: ${result.questions?.length || 0}`,
      );

      return result;
    } catch (error) {
      console.error(
        "[QuizApiService.mapResponseToQuiz] HATA: Quiz dönüştürme işlemi başarısız!",
        error,
      );
      throw new Error(
        `Quiz dönüştürme hatası: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async generateQuiz(options: QuizGenerationOptions): Promise<Quiz> {
    try {
      ...
      const relevantTypesForSubtopicValidation: QuizType[] = [
        "personalized",
        "general",
        "topic_specific",
      ];
      if (
        relevantTypesForSubtopicValidation.includes(options.quizType) ||
        (options.quizType as string) === "quick"
      ) {
        // Type assertion added to allow 'quick' string comparison
        if (!subTopicIds || subTopicIds.length === 0) {
          const clientError = ErrorService.createApiError(
            ...
            "Sınav oluşturmak için en az bir alt konu seçilmelidir. Lütfen Konu Seçimi adımında alt konu seçtiğinizden emin olun.",
            {
              original: {
                error: new Error(
                  `Client-side validation: subTopics array is empty for ${options.quizType} quiz type.`,
                ),
      ...
      if (options.quizType === "personalized") {
        ...
        if (!finalSubTopics.length) {
          ...
          console.warn(
            "[QuizApiService.generateQuiz] subTopics dizisi boş, varsayılan konular ekleniyor",
          );
        }
        ...
        payload = {
          ...
          description:
            options.description ||
            "Otomatik oluşturulan kişiselleştirilmiş sınav",
          ...
          prioritizeWeakTopics:
            options.preferences.prioritizeWeakAndMediumTopics || false,
        };

        // Hata ayıklama: Payload içeriğini detaylı logla
        console.log(
          "[QuizApiService.generateQuiz] Kişiselleştirilmiş sınav isteği detayları:",
          {
            endpoint,
            documentId: options.documentId,
            courseId: courseId,
        ...
        if (!courseId || !finalSubTopics.length) {
          console.error(
            "[QuizApiService.generateQuiz] HATA: Zorunlu alanlar eksik",
            { courseId, subTopicsCount: finalSubTopics.length },
          );
        } else {
          console.log(
            "[QuizApiService.generateQuiz] İstek formatı doğrulandı, zorunlu alanlar mevcut",
          );
        }
      } else {
        // Handles 'quick', 'general', 'topic_specific'
        ...
        payload = {
          ...
          timeLimit: options.preferences.timeLimit,
          prioritizeWeakTopics:
            options.preferences.prioritizeWeakAndMediumTopics,
        };
      }

      // API isteğini gönder
      const response = await apiService.post<QuizResponseDto>(
        endpoint,
        payload,
      );
      ...
      try {
        ...
        console.log(
          `[QuizApiService.generateQuiz] İşlenen API yanıtı:`,
          responseData,
        );
      } catch (error) {
        console.error(
          `[QuizApiService.generateQuiz] API yanıtı işlenirken hata:`,
          error,
        );
      ...
      console.log(
        `[QuizApiService.generateQuiz] API yanıtı başarılı:`,
        responseData,
      );
      ...
      if (!responseData || !this.isObject(responseData)) {
        console.warn(
          "[QuizApiService.generateQuiz] API yanıtı beklenen formatta değil, varsayılan sınav oluşturuluyor",
        );
        console.warn(
          "[QuizApiService.generateQuiz] Problem olan yanıt:",
          responseData,
        );
      ...
      if (!responseData.id) {
        console.warn(
          "[QuizApiService.generateQuiz] API yanıtı id içermiyor, muhtemelen farklı bir formatta. Yanıt:",
          responseData,
        );
        console.log(
          "[QuizApiService.generateQuiz] Yanıt anahtarları:",
        ...
        if (
          responseData.data &&
          this.isObject(responseData.data) &&
          responseData.data.id
        ) {
          console.log(
            "[QuizApiService.generateQuiz] API yanıtı içinde data nesnesi bulundu:",
            responseData.data,
          );
          try {
            const mappedQuiz = this.mapResponseToQuiz(
              responseData.data as unknown as QuizResponseDto,
            );
            ...
            console.error(
              "[QuizApiService.generateQuiz] Data nesnesi dönüştürme hatası:",
              dataMapError,
            );
        ...
        console.log(
          "[QuizApiService.generateQuiz] Alternatif yapılar aranıyor...",
        );

        if (this.isObject(responseData.quiz)) {
          console.log(
            "[QuizApiService.generateQuiz] 'quiz' anahtarı bulundu:",
            responseData.quiz,
          );
          possibleQuiz = responseData.quiz;
        } else if (this.isObject(responseData.result)) {
          console.log(
            "[QuizApiService.generateQuiz] 'result' anahtarı bulundu:",
            responseData.result,
          );
          possibleQuiz = responseData.result;
        } else if (Array.isArray(responseData.questions)) {
          console.log(
            "[QuizApiService.generateQuiz] 'questions' dizisi bulundu, manuel sınav oluşturuluyor",
          );
          ...
          console.log(
            "[QuizApiService.generateQuiz] Manuel oluşturulan quiz:",
            possibleQuiz,
          );
        }
        ...
        if (possibleQuiz && possibleQuiz.id) {
          console.log(
            "[QuizApiService.generateQuiz] Alternatif formatta quiz bulundu:",
            possibleQuiz,
          );
          try {
            const mappedQuiz = this.mapResponseToQuiz(
              possibleQuiz as unknown as QuizResponseDto,
            );
            return mappedQuiz;
          } catch (altMapError) {
            console.error(
              "[QuizApiService.generateQuiz] Alternatif format dönüştürme hatası:",
              altMapError,
            );
            // Fallback'e devam et
          }
        ...
        console.warn(
          "[QuizApiService.generateQuiz] Geçerli bir sınav formatı bulunamadı, varsayılan oluşturuldu",
        );
        const fallbackQuiz = this.createFallbackQuiz(
          "format_fallback",
          options,
        );
        return fallbackQuiz;
      ...
      try {
        ...
        console.log(
          "[QuizApiService.generateQuiz] Standart yanıt işleniyor. ID:",
          responseData.id,
        );
        const mappedQuiz = this.mapResponseToQuiz(
          responseData as unknown as QuizResponseDto,
        );
        console.log(
          "[QuizApiService.generateQuiz] Quiz başarıyla oluşturuldu:",
          mappedQuiz.id,
        );
        ...
        console.error(
          "[QuizApiService.generateQuiz] Quiz dönüşümü sırasında hata:",
          mapError,
        );
        console.error(
          "[QuizApiService.generateQuiz] Dönüşüm hatası detayları:",
          {
            errorType:
              mapError instanceof Error
                ? mapError.constructor.name
                : typeof mapError,
            message:
      ...
      console.error(
        `[QuizApiService.generateQuiz] HATA OLUŞTU: ${error instanceof Error ? error.message : String(error)}`,
      );
      ...
      const isInvalidJsonResponse = errorMessageString.includes(
        "AI response is not valid JSON",
      );

      if (isInvalidJsonResponse) {
        console.warn(
          "[QuizApiService.generateQuiz] AI response is not valid JSON detected.",
        );

        // Only retry for personalized quizzes with multiple subtopics that can be reduced
        if (
          options.quizType === "personalized" &&
          Array.isArray(options.selectedSubTopics) &&
          options.selectedSubTopics.length > 1
        ) {
          console.log(
            `[QuizApiService.generateQuiz] Attempting to regenerate quiz with ${options.selectedSubTopics.length - 1} subtopics.`,
          );
          ...
          try {
            console.log(
              "[QuizApiService.generateQuiz] Retrying quiz generation with new options:",
              newOptions,
            );
            ...
            const retryErrorMessage =
              retryError instanceof Error
                ...
                : String(retryError);
            console.error(
              `[QuizApiService.generateQuiz] Retry with fewer subtopics failed: ${retryErrorMessage}`,
              retryError,
            );
            const fallbackQuiz = this.createFallbackQuiz(
              "retry_json_fallback",
          ...
          let reason =
            "Cannot reduce subtopics or not a personalized quiz with multiple subtopics.";
          if (options.quizType !== "personalized")
            reason = "Not a personalized quiz, retry not applicable.";
          else if (
            !options.selectedSubTopics ||
            !Array.isArray(options.selectedSubTopics) ||
            options.selectedSubTopics.length <= 1
          )
            reason = "Not enough subtopics to reduce for retry.";

          console.warn(
            `[QuizApiService.generateQuiz] ${reason} Generating fallback quiz due to invalid JSON.`,
          );
          const fallbackQuiz = this.createFallbackQuiz(
        ...
        console.error(
          `[QuizApiService.generateQuiz] Axios hatası: Status=${statusCode}, Data=`,
          responseData,
        );
        ...
        console.error(
          "[QuizApiService.generateQuiz] Genel veya bilinmeyen bir hata oluştu.",
          error,
        );
  ...
  async submitQuiz(
    ...
    if (!payload.quizId || payload.quizId === "undefined") {
      console.warn(
        "[QuizApiService.submitQuiz] Geçersiz Quiz ID:",
        payload.quizId,
      );
      ErrorService.showToast(
        "Sınav ID'si bulunamadı, sonuçlar yerel olarak işlenecek",
        "warning",
        "Sınav Gönderimi",
      );
    ...
    if (!payload.userAnswers || Object.keys(payload.userAnswers).length === 0) {
      console.warn(
        "[QuizApiService.submitQuiz] Kullanıcı cevapları eksik veya boş:",
        payload.userAnswers,
      );
      ErrorService.showToast(
        "Sınav cevapları bulunamadı",
        "error",
        "Sınav Gönderimi",
      );
      return this.createLocalQuizSubmission(payload);
    ...
    try {
      return await this.apiService.safeRequest(
        ...
        async () => {
          try {
            ...
            if (!storedQuiz) {
              console.warn(
                "[QuizApiService.submitQuiz] LocalStorage'da quiz bulunamadı, minimum veri ile devam ediliyor",
              );
              ErrorService.showToast(
                "Sınav verileri eksik, basit analiz yapılacak",
                "warning",
                "Sınav Gönderimi",
              );
            ...
            console.log(
              "[QuizApiService.submitQuiz] Hazırlanan sorular:",
              preparedQuestions,
            );
            ...
            const payloadForSubmission = formattedPayload as unknown as Record<
              string,
              unknown
            >;
            ...
            if (Array.isArray(payloadForSubmission.questions)) {
              (
                payloadForSubmission.questions as Record<string, unknown>[]
              ).forEach((q) => {
                // Alanları temizle
                ...
                if (
                  q.normalizedSubTopic === null ||
                  q.normalizedSubTopic === undefined
                ) {
              ...
              console.warn(
                "[QuizApiService.submitQuiz] questions dizisi bulunamadı veya dizi değil",
              );
              payloadForSubmission.questions = []; // Boş dizi ata
            }

            console.log(
              "[QuizApiService.submitQuiz] Backend'e gönderilecek formatlı veri (adapter ile):",
              payloadForSubmission,
            );
            ...
            try {
              ...
              console.log(
                `[QuizApiService.submitQuiz] API isteği gönderiliyor: ${endpoint}`,
              );

              const result = await this.apiService.post<QuizSubmissionResponse>(
                endpoint,
                payloadForSubmission,
              );
              console.log(
                "[QuizApiService.submitQuiz] API yanıtı başarılı:",
                result,
              );
              ...
              if (axios.isAxiosError(error)) {
                console.error(
                  "[QuizApiService.submitQuiz] Status:",
                  error.response?.status,
                );
                console.error(
                  "[QuizApiService.submitQuiz] Response data:",
                  error.response?.data,
                );
                ...
                console.warn(
                  `[QuizApiService.submitQuiz] Backend hatası, lokalde sonuç oluşturuluyor`,
                );
                return this.createLocalQuizSubmission(payload);
              }
        ...
        () => {
          ...
          ErrorService.showToast(
            "Bağlantı sorunu, yerel analiz kullanılıyor",
            "info",
            "Sınav Gönderimi",
          );
          return this.createLocalQuizSubmission(payload);
      ...
      ErrorService.showToast(
        "Beklenmeyen bir hata oluştu, yerel analiz kullanılıyor",
        "error",
        "Sınav Gönderimi",
      );
      return this.createLocalQuizSubmission(payload);
  ...
  private prepareQuestionsForSubmission(
    questions: Question[],
    ...
    if (!Array.isArray(questions) || questions.length === 0) {
      console.warn(
        "[QuizApiService.prepareQuestionsForSubmission] Sorular bulunamadı veya geçersiz format, varsayılan sorular oluşturuluyor",
      );
      // Kullanıcı yanıtlarından soru ID'lerini al ve varsayılan sorular oluştur
      const fallbackQuestions = Object.keys(userAnswers || {}).map(
        (qId, index) => ({
          id: qId,
    ...
    const preparedQuestions = questions.map((q, index) => {
      try {
        ...
        if (!question.id) {
          ...
          console.log(
            `[QuizApiService.prepareQuestionsForSubmission] Soru #${index} için ID oluşturuldu: ${question.id}`,
          );
        }
        ...
        if (
          ...
          console.log(
            `[QuizApiService.prepareQuestionsForSubmission] Soru #${index} için metin oluşturuldu`,
          );
        }
        ...
        if (!question.subTopic || typeof question.subTopic !== "string") {
          ...
          console.log(
            `[QuizApiService.prepareQuestionsForSubmission] Soru #${index} için subTopic oluşturuldu`,
          );
        }

        // normalizedSubTopic kontrolü ve oluşturma
        if (
          !question.normalizedSubTopic ||
          typeof question.normalizedSubTopic !== "string"
        ) {
          try {
            ...
            question.normalizedSubTopic = question.subTopic
              ...
              .replace(/[^a-z0-9\-]/g, "");
            console.log(
              `[QuizApiService.prepareQuestionsForSubmission] Soru #${index} için normalizedSubTopic oluşturuldu: ${question.normalizedSubTopic}`,
            );
          } catch (error) {
            // Herhangi bir hata durumunda varsayılan değer kullan
            console.error(
              `[QuizApiService.prepareQuestionsForSubmission] normalizedSubTopic oluştururken hata:`,
              error,
            );
        ...
        if (typeof question.normalizedSubTopic !== "string") {
          question.normalizedSubTopic = String(
            question.normalizedSubTopic || "genel-konu",
          );
        }
        ...
        if (!Array.isArray(question.options) || question.options.length < 2) {
          ...
          console.log(
            `[QuizApiService.prepareQuestionsForSubmission] Soru #${index} için seçenekler oluşturuldu`,
          );
        }
        ...
        if (
          ...
          console.log(
            `[QuizApiService.prepareQuestionsForSubmission] Soru #${index} için doğru cevap ayarlandı: ${question.correctAnswer}`,
          );
        }
        ...
        console.error(
          `[QuizApiService.prepareQuestionsForSubmission] Soru #${index} işlenirken hata:`,
          error,
        );
  ...
  async saveQuickQuiz(quiz: Quiz): Promise<Quiz> {
    try {
      ...
      const response = await apiService.post<ApiQuiz>(
        API_ENDPOINTS.SAVE_QUICK_QUIZ,
        quizAsRecord,
      );
      return adapterService.toQuiz(response);
    } catch (error) {
      console.error(
        "[QuizApiService.saveQuickQuiz] Sınav kaydedilirken hata:",
        error,
      );
  ...
  private createLocalQuizSubmission(
    payload: QuizSubmissionPayload,
  ): QuizSubmissionResponse {
    console.info(
      "[QuizApiService] Backend bağlantısı kurulamadı, yerel sınav sonuçları oluşturuluyor",
      payload,
    );
    ...
    if (!storedQuiz) {
      console.warn(
        "[QuizApiService] Yerel depolama verisi bulunamadı:",
        currentQuizId,
      );
    ...
    try {
      ...
      if (currentQuizId !== undefined) {
        // Avoid key "quizResult_undefined" if possible, though it works
        localStorage.setItem(
          `quizResult_${currentQuizId}`,
          JSON.stringify(storedQuiz),
        );
      } else {
        console.warn(
          "[QuizApiService] currentQuizId is undefined, cannot save to localStorage with specific key.",
        );
      }
    } catch (e) {
    ...
    const correctCount = Object.entries(safeUserAnswersForScoring).filter(
      ([qId, ans]) => {
    ...
    const totalQuizQuestions = storedQuiz.questions
      ? storedQuiz.questions.length
      : 0;
    const score =
      totalQuizQuestions > 0
        ? Math.round((correctCount / totalQuizQuestions) * 100)
        : 0;
  ...
  private generateLocalAnalysis(quizId: string): QuizAnalysisResponse {
    ...
    storedQuiz.questions.forEach((q) => {
      ...
      const subTopic =
        q.subTopic || q.normalizedSubTopic || "Alt Konu Belirtilmemiş";
    ...
    Object.keys(performanceBySubTopic).forEach((topic) => {
      ...
      perf.scorePercent = Math.round(
        (perf.correctCount / perf.questionCount) * 100,
      );
    ...
    const overallScore = Math.round(
      (totalCorrect / storedQuiz.questions.length) * 100,
    );
...
export async function fetchExamResultFromBackend(quizId: string) {
  console.log(
    "[BACKEND_FETCH_TRACE] fetchExamResultFromBackend başlatıldı, quizId:",
    quizId,
  );
  try {
    ...
    if (result === undefined) {
      console.warn(
        "[BACKEND_FETCH_WARN] Firestore getDocument sonucu undefined",
      );
    } else if (result === null) {
      console.warn("[BACKEND_FETCH_WARN] Firestore getDocument sonucu null");
    } else if (typeof result !== "object") {
      console.warn(
        "[BACKEND_FETCH_WARN] Firestore getDocument tipi:",
        typeof result,
      );
    } else {
      console.log(
        "[BACKEND_FETCH_TRACE] Firestore getDocument tip kontrolü geçti:",
        result,
      );
      if (result.id)
        console.log("[BACKEND_FETCH_TRACE] Backend veri id:", result.id);
      if (result.title)
        console.log("[BACKEND_FETCH_TRACE] Backend veri title:", result.title);
      if (result.quizTitle)
        console.log(
          "[BACKEND_FETCH_TRACE] Backend veri quizTitle:",
          result.quizTitle,
        );
      if (Array.isArray(result.questions))
        console.log(
          "[BACKEND_FETCH_TRACE] Backend veri questions uzunluğu:",
