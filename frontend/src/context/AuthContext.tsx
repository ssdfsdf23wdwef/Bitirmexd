...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const setFirebaseUser = useAuthStore(
    (state: AuthState) => state.setFirebaseUser,
  );
        ...
        try {
          ...
          trackFlow(
            firebaseUser
              ...
              : "Kullanıcı oturumu değişikliği algılandı",
            ...
            try {
              ...
              if (axios.isAxiosError(error)) {
                ...
                // Diğer API hataları
                  ...
                  setUser(null);
                  setAuthError(
                    `API hatası: ${axiosError.response?.statusText || "Bilinmeyen hata"}`,
                  );
                }
              } else {
                // Diğer hatalar
                ...
                setUser(null);
                setAuthError(
                  "Oturum bilgileri alınırken beklenmeyen bir hata oluştu.",
                );
              }
  ...
  const handleConnectionError = (firebaseUser: FirebaseUser) => {
    ...
    setAuthError(
      "Sunucu bağlantısı kurulamadı. Çevrimdışı modda sınırlı işlemler yapabilirsiniz.",
    );
  ...
  const checkSession = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (
        ...
        trackFlow(
          "Oturum kontrolü bağlantı hatası (offline?)",
          "AuthContext.checkSession",
          FlowCategory.Error,
        );
  ...
  const login = useCallback(
    async (email: string, password: string): Promise<AuthResponse> => {
      ...
      try {
        ...
        trackFlow(
          "Backend ile ID token doğrulanıyor",
          "AuthContext.login",
          FlowCategory.Auth,
        );
        ...
        trackFlow("Giriş hatası", "AuthContext.login", FlowCategory.Error, {
          errorMessage,
        });
  ...
  const loginWithGoogle = useCallback(async (): Promise<GoogleAuthResponse> => {
    logger.debug(
      "Google ile login isteği başlatıldı",
    ...
    try {
      ...
      trackFlow(
        "Backend'e ID token gönderiliyor",
        "AuthContext.loginWithGoogle",
        FlowCategory.Auth,
      );
      ...
      trackFlow(
        "Google ile giriş backend doğrulaması başarılı",
        "AuthContext.loginWithGoogle",
        FlowCategory.Auth,
      );
      ...
      if (
        ...
        logger.warn(
          "Google login popup kullanıcı tarafından kapatıldı",
          "AuthContext.loginWithGoogle",
          "AuthContext.tsx",
          307,
        );
  ...
  const signOut = useCallback(async (): Promise<boolean> => {
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Oturum kapatılırken bilinmeyen bir hata oluştu";
      trackFlow("SignOut hatası", "AuthContext.signOut", FlowCategory.Error, {
        error,
      });
      logger.error(
        "SignOut hatası",
  ...
  const resetPassword = useCallback(async (email: string): Promise<boolean> => {
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Şifre sıfırlama sırasında bilinmeyen bir hata oluştu";
      trackFlow(
        "ResetPassword hatası",
        "AuthContext.resetPassword",
        FlowCategory.Error,
        { error, email },
      );
    ...
    () => ({
      ...
      register: async (
        ...
        try {
          ...
          logger.info(
            "Kayıt başarılı, ana sayfaya yönlendiriliyor",
            "AuthContext.register",
            "AuthContext.tsx",
            600,
          );
