...
export default function SettingsPage() {
  ...
  const inputClassName =
    "w-full text-sm px-3 py-2.5 bg-white dark:bg-[#1e293b] border border-[#e2e8f0] dark:border-[#334155] rounded-md text-[#0f172a] dark:text-[#f8fafc] focus:ring-2 focus:ring-[#3b82f6]/50 dark:focus:ring-[#60a5fa]/50 focus:border-transparent outline-none transition-colors";

  // Toggle butonu stilini merkezi olarak tanımla
  const toggleClassName =
    "w-10 h-5 bg-[#f1f5f9] dark:bg-[#1e293b] peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-[#3b82f6]/30 dark:peer-focus:ring-[#3b82f6]/40 rounded-full peer dark:bg-border-secondary peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white dark:after:bg-white after:border-[#e2e8f0] dark:after:border-[#334155] after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-[#3b82f6] dark:peer-checked:bg-[#3b82f6]";
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-10">
        <div className="md:col-span-2 space-y-8">
          <motion.section variants={fadeInVariants} className={cardClassName}>
            ...
            <div
              className={`rounded-md p-4 mb-6 flex items-center gap-4 border ${isDarkMode ? "bg-gray-900/80 border-gray-700/40" : "bg-gray-50 border-gray-200/70"} transition-colors duration-300`}
            >
              {isDarkMode ? (
                ...
                {(["light", "dark", "system"] as const).map((mode) => (
                  ...
                  >
                    <div className="flex flex-col items-center gap-1.5">
                      {mode === "light" && (
                        <FiSun
                          className={`text-lg ${theme.mode === mode ? "text-[#f59e0b] dark:text-[#fbbf24]" : "text-[#64748b] group-hover:text-[#f59e0b] dark:group-hover:text-[#fbbf24]"}`}
                        />
                      )}
                      {mode === "dark" && (
                        <FiMoon
                          className={`text-lg ${theme.mode === mode ? "text-[#3b82f6] dark:text-[#60a5fa]" : "text-[#64748b] group-hover:text-[#3b82f6] dark:group-hover:text-[#60a5fa]"}`}
                        />
                      )}
                      {mode === "system" && (
                        <FiMonitor
                          className={`text-lg ${theme.mode === mode ? "text-[#0f172a] dark:text-[#f8fafc]" : "text-[#64748b] group-hover:text-[#0f172a] dark:group-hover:text-[#f8fafc]"}`}
                        />
                      )}
                      <span
                        className={`text-xs font-medium ${theme.mode === mode ? "text-[#2563eb] dark:text-[#93c5fd]" : "text-[#64748b] group-hover:text-[#0f172a] dark:group-hover:text-[#f8fafc]"} capitalize`}
                      >
                        {mode === "light"
                          ? "Açık"
                          : mode === "dark"
                ...
                {(["small", "medium", "large"] as const).map((size) => (
                  ...
                  >
                    <div className="flex flex-col items-center gap-1.5">
                      ...
                      <span
                        className={`text-xs font-medium ${theme.fontSize === size ? "text-[#10b981] dark:text-[#34d399]" : "text-[#64748b] group-hover:text-[#0f172a] dark:group-hover:text-[#f8fafc]"} capitalize`}
                      >
                        {size === "small"
                          ? "Küçük"
                          : size === "medium"
          ...
          <motion.section variants={fadeInVariants} className={cardClassName}>
            ...
            <div className="space-y-5">
              {(
                ["emailNotifications", "autoStartTimer", "showHints"] as const
              ).map((key) => {
                const labels = {
                ...
                return (
                  <div
                    key={key}
                    className={`flex items-center justify-between p-3.5 rounded-md border ${isDarkMode ? "bg-gray-900/80 border-gray-700/40" : "bg-white border-gray-200/70"} transition-colors duration-300`}
                  >
                    <div>
              ...
              <div
                className={`p-3.5 rounded-md border ${isDarkMode ? "bg-gray-900/80 border-gray-700/40" : "bg-white border-gray-200/70"} transition-colors duration-300`}
              >
                <label
                  htmlFor="language"
          ...
          <motion.section variants={fadeInVariants} className={cardClassName}>
            ...
            <div className="space-y-4">
              {(
                Object.keys(passwordData) as Array<keyof typeof passwordData>
              ).map((field) => {
                const labels = {
                ...
                const fieldName = field
                  .replace("Password", "")
                  .toLowerCase() as keyof typeof showField;
                return (
                  <div key={field}>
                    <label
                      htmlFor={field}
        ...
        <aside className="md:col-span-1">
          ...
          >
            ...
            <p className="text-sm text-[#334155] dark:text-[#cbd5e1] mb-5">
              Ayarlar veya hesap yönetimi ile ilgili sorun yaşıyorsanız, destek
              merkezimizi ziyaret edebilir veya bizimle iletişime
              geçebilirsiniz.
            </p>
            <a
              href="#"
