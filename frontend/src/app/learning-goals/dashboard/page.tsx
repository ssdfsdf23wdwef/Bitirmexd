      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
        ...
        >
          <h2 className={`text-xl font-semibold mb-6 flex items-center ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>
            <div className={`p-2 rounded-lg mr-3 ${isDarkMode ? 'bg-blue-500/25 text-blue-300 border border-blue-400/30' : 'bg-indigo-100 text-indigo-600'}`}>
              <FiPieChart className="w-6 h-6" aria-hidden="true" />
            </div>
            Hedef Durumu Dağılımı
          </h2>
          <div className="h-72 flex items-center justify-center" aria-label="Hedef durumlarının pasta grafiği">
        ...
        >
          ...
          <div className="h-72" aria-label="Sınav skorlarının çubuk grafiği">
            {barChartData ? (
              <Bar
                data={barChartData}
                options={{
      ...
      >
        <h2 className={`text-xl font-semibold mb-6 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>
          ...
          <span className={`ml-2 px-3 py-1 rounded-full text-xs font-medium ${
            isDarkMode ? 'bg-blue-500/25 text-blue-300 border border-blue-400/30' : 'bg-indigo-100 text-indigo-600'
          }`}>
        ...
        {learningTargets.length === 0 ? (
          <div className="text-center py-12">
            <div className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center ${
              isDarkMode ? 'bg-slate-700/60 border border-slate-600/50' : 'bg-gray-100'
            }`}>
            ...
            <p className={`text-lg font-medium mb-2 ${isDarkMode ? 'text-gray-200' : 'text-gray-600'}`}>
          ...
          <div className="overflow-hidden rounded-xl">
            <div className="overflow-x-auto">
              <table
                ...
                aria-label="Öğrenme hedefleri tablosu"
              >
                <thead className={`${isDarkMode ? 'bg-slate-700/60 border-b border-slate-600/50' : 'bg-gray-50/80'} backdrop-blur-sm`}>
                  <tr>
                    <th
                      scope="col"
                ...
                <tbody className={`divide-y ${isDarkMode ? 'divide-slate-700/50' : 'divide-gray-200/50'}`}>
                          ...
                          target.lastAttemptScorePercent !== undefined ? (
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                              target.lastAttemptScorePercent >= 80
                                ? isDarkMode ? 'bg-emerald-500/25 text-emerald-300 border border-emerald-400/30' : 'bg-green-100 text-green-800'
                                : target.lastAttemptScorePercent >= 60
                                ? isDarkMode ? 'bg-amber-500/25 text-amber-300 border border-amber-400/30' : 'bg-yellow-100 text-yellow-800'
                                : isDarkMode ? 'bg-red-500/25 text-red-300 border border-red-400/30' : 'bg-red-100 text-red-800'
                            }`}>
