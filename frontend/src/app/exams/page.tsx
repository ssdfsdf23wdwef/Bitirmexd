    ...
    >
      <td className="px-4 py-3 whitespace-nowrap">
        <div className="flex items-center">
          ...
          <div>
            <div
              className={`text-sm font-semibold truncate max-w-xs ${isDarkMode ? "text-gray-100" : "text-gray-900"}`}
            >
              {title}
            </div>
            <div
      ...
      <td className="px-4 py-3 whitespace-nowrap">
        <div className="flex items-center">
          <FiCalendar
            className={`w-3 h-3 mr-1 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
          />
          <span
...
export default function ExamsPage() {
  ...
  if (isError || !quizzes) {
    return (
      ...
      >
        <div
          className={`p-8 rounded-2xl border backdrop-blur-sm text-center max-w-md ${
            isDarkMode
    ...
    >
      ...
      {filteredQuizzes.length > 0 ? (
        ...
        >
          ...
          {/* Mobil kart görünümü */}
          <div
            className={`lg:hidden max-h-[calc(100vh-200px)] overflow-y-auto p-3 space-y-3 ${
              isDarkMode ? "scrollbar-dark" : "scrollbar-light"
            }`}
            style={{
              scrollbarWidth: "thin",
          ...
          >
            {filteredQuizzes.map((quiz, idx) => {
              ...
              const questionsCount = quiz.totalQuestions;
              const createdAt = quiz.timestamp;
              const duration = quiz.elapsedTime
                ? Math.round(quiz.elapsedTime / 60)
                : "-";
        ...
        >
          <div className="w-32 h-32 relative mb-8">
            <div
              className={`absolute inset-0 opacity-20 rounded-3xl rotate-6 ${
                isDarkMode
            ...
            ></div>
            <div
              className={`absolute inset-0 rounded-3xl flex items-center justify-center border backdrop-blur-sm ${
                isDarkMode
                  ? "bg-slate-800/80 border-slate-700/50"
                  : "bg-white/80 border-gray-200/50"
              }`}
            >
              <FiFileText
                className={`text-6xl bg-gradient-to-br ${
                  isDarkMode
                    ? "from-blue-400 to-purple-400"
                    : "from-blue-600 to-purple-600"
                } bg-clip-text text-transparent`}
