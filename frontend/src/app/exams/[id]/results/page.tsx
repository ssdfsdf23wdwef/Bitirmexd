...
const calculateResults = (
  ...
  console.log(
    "[RESULTS_PAGE_TRACE] calculateResults() ÇAĞRILDI. Parametreler:",
    { questions, userAnswers, quizTitle },
  );
  let correctCount = 0;
  const processedQuestions: Question[] = questions.map((q) => {
    ...
    console.log(
      `[RESULTS_PAGE_TRACE] Soru: ${q.id} | Kullanıcı Cevabı: ${userAnswer} | Doğru Cevap: ${q.correctAnswer} | isCorrect: ${isCorrect}`,
    );
  ...
  console.log(
    `[RESULTS_PAGE_TRACE] Toplam Doğru: ${correctCount} / ${questions.length} | Skor: ${overallScore}`,
  );
  const subTopicStatsMap: Map<string, { correct: number; total: number }> =
    new Map();
  processedQuestions.forEach((q) => {
    ...
    console.log(
      `[RESULTS_PAGE_TRACE] Alt Konu: ${q.subTopic} | Doğru: ${stat.correct} | Toplam: ${stat.total}`,
    );
  });
  const subTopicStats = Array.from(subTopicStatsMap.entries()).map(
    ([subTopic, data]) => ({
      subTopic,
  ...
  console.log(
    "[RESULTS_PAGE_TRACE] calculateResults() DÖNDÜRÜLEN SONUÇ:",
    result,
  );
  return result;
};
...
export default function ExamResultsPage() {
  ...
  function normalize(val: any) {
    ...
    if (typeof val === "object" && "text" in val)
      return String(val.text).trim().toLocaleLowerCase("tr");
  ...
  const calculatedSubTopicStats: Array<{
    subTopic: string;
    score: number;
    totalQuestions: number;
    correctQuestions: number;
    ...
    quizResult.questions.forEach((q) => {
      ...
      if (!statsMap[q.subTopic])
        statsMap[q.subTopic] = { total: 0, correct: 0 };
  ...
  const calculatedOverallScore: number = React.useMemo(() => {
    if (
      !quizResult ||
      !Array.isArray(quizResult.questions) ||
  ...
  useEffect(() => {
    ...
    (async () => {
      console.log(
        `[RESULTS_PAGE_TRACE] useEffect BAŞLADI. params.id=`,
        params.id,
        "| quizResult exists:",
        !!quizResult,
      ...
      if (dataLoadedRef.current) {
        console.log(
          "[RESULTS_PAGE_TRACE] dataLoadedRef.current=true olduğu için veri tekrar yüklenmeyecek. useEffect çıkıyor.",
        );
        return;
      }
      if (quizResult) {
        dataLoadedRef.current = true;
        console.log(
          "[RESULTS_PAGE_TRACE] quizResult yüklendi, dataLoadedRef.current=true olarak ayarlandı. useEffect çıkıyor.",
        );
        return;
      }
      console.log(
        "[RESULTS_PAGE_TRACE] Sonuçlar henüz yüklenmemiş, yükleme işlemi başlatılıyor. State'ler:",
        { loading, dataError, quizResult },
      );
      setLoading(true);
      setDataError(null);
      ...
      try {
        const currentQuizId = Array.isArray(params.id)
          ? params.id[0]
          : (params.id as string);
        console.log(
          `[RESULTS_PAGE_TRACE] useEffect: quizId oluşturuldu: ${currentQuizId}`,
        );
        if (!currentQuizId) {
          console.error(
            "[RESULTS_PAGE_ERROR] Sınav ID bulunamadı. params:",
            params,
          );
          setDataError("Sınav ID bulunamadı.");
          dataLoadedRef.current = true;
          console.log(
            "[RESULTS_PAGE_STATE] Hata durumunda dataLoadedRef.current=true olarak ayarlandı.",
          );
          setLoading(false);
          return;
        }
        ...
        console.log(
          `[RESULTS_PAGE_TRACE] 🔑 localStorage'dan okunacak anahtar: ${storageKeyUsed}`,
        );
        console.log(
          `[RESULTS_PAGE_TRACE] localStorage.getItem(${storageKeyUsed}) ÇAĞRILIYOR.`,
        );
        storedDataString = localStorage.getItem(storageKeyUsed);
        console.log(
          `[RESULTS_PAGE_TRACE] 📄 localStorage'dan okunan veri (string):`,
          storedDataString,
        );
        if (!storedDataString) {
          // Local'de veri yoksa backend'den çekmeye çalış
          console.log(
            `[RESULTS_PAGE_DEBUG] LocalStorage'da veri yok, backend'den sonuç çekilecek: ${storageKeyUsed}`,
          );
          setLoading(true);
          console.log(
            `[RESULTS_PAGE_TRACE] setLoading(true) çağrıldı (backend fetch öncesi). Şu anki state:`,
            { loading, dataError, quizResult },
          );
          try {
            console.log(
              `[RESULTS_PAGE_TRACE] fetchExamResultFromBackend(${currentQuizId}) çağrılıyor...`,
            );
            const backendResult =
              await fetchExamResultFromBackend(currentQuizId);
            console.log(
              `[RESULTS_PAGE_TRACE] fetchExamResultFromBackend dönüşü:`,
              backendResult,
            );
            if (backendResult === undefined) {
              ...
              console.warn(
                "[RESULTS_PAGE_WARN] Backend sonuç tip hatası:",
                typeof backendResult,
              );
            } else {
              console.log(
                "[RESULTS_PAGE_TRACE] Backend sonuç tip kontrolü geçti:",
                backendResult,
              );
              if (backendResult.id)
                console.log(
                  "[RESULTS_PAGE_TRACE] Backend veri id:",
                  backendResult.id,
                );
              if (backendResult.title)
                console.log(
                  "[RESULTS_PAGE_TRACE] Backend veri title:",
                  backendResult.title,
                );
              if (backendResult.quizTitle)
                console.log(
                  "[RESULTS_PAGE_TRACE] Backend veri quizTitle:",
                  backendResult.quizTitle,
                );
              if (Array.isArray(backendResult.questions))
                console.log(
                  "[RESULTS_PAGE_TRACE] Backend veri questions uzunluğu:",
                  backendResult.questions.length,
                );
            }
            if (isMounted && backendResult) {
              console.log(
                "[RESULTS_PAGE_TRACE] setQuizResult() çağrılacak. Önceki quizResult:",
                quizResult,
              );
              setQuizResult((prev) => {
                console.log(
                  "[RESULTS_PAGE_TRACE] setQuizResult callback, önceki:",
                  prev,
                  "yeni:",
                  backendResult,
                );
              ...
              setTimeout(() => {
                console.log(
                  "[RESULTS_PAGE_TRACE] setQuizResult sonrası quizResult:",
                  quizResult,
                );
              }, 100);
              dataLoadedRef.current = true;
              console.log(
                `[RESULTS_PAGE_TRACE] dataLoadedRef.current=true olarak ayarlandı (backend success).`,
              );
            } else if (isMounted) {
              setDataError("Sonuçlar backend'den alınamadı.");
              console.log(
                `[RESULTS_PAGE_ERROR] Backend'den sonuç alınamadı veya veri yok. State güncellendi.`,
              );
              setTimeout(() => {
                console.log(
                  "[RESULTS_PAGE_TRACE] setDataError sonrası dataError:",
                  dataError,
                );
              }, 100);
              dataLoadedRef.current = true;
              console.log(
                `[RESULTS_PAGE_TRACE] dataLoadedRef.current=true olarak ayarlandı (backend error).`,
              );
            }
          } catch (err) {
            if (isMounted) {
              setDataError("Sonuçlar backend'den alınamadı.");
              console.error(
                `[RESULTS_PAGE_ERROR] Backend fetch sırasında hata oluştu:`,
                err,
              );
              if (err instanceof Error) {
                console.error(
                  "[RESULTS_PAGE_ERROR] Backend fetch hata stack:",
                  err.stack,
                );
              }
              setTimeout(() => {
              ...
              dataLoadedRef.current = true;
              console.log(
                `[RESULTS_PAGE_TRACE] dataLoadedRef.current=true olarak ayarlandı (backend catch bloğu).`,
              );
            }
          }
          if (isMounted) {
            setLoading(false);
            console.log(
              `[RESULTS_PAGE_TRACE] setLoading(false) çağrıldı (backend fetch sonrası). Şu anki state:`,
              { loading, dataError, quizResult },
            );
          }
          return;
        ...
        try {
          ...
          console.log(
            "[RESULTS_PAGE_TRACE] 📦 localStorage'dan parse edilen veri:",
            parsedData,
          );
        } catch (parseErr) {
          console.error(
            "[RESULTS_PAGE_ERROR] JSON.parse HATASI:",
            parseErr,
            "| Okunan veri:",
            storedDataString,
          );
        ...
        const {
          ...
          userAnswersData: parsedUserAnswersData,
        } = parsedData;

        if (
          !parsedQuizData ||
          !parsedQuizData.questions ||
          !parsedUserAnswersData
        ) {
          console.error(
            "[RESULTS_PAGE_ERROR] Alınan sınav verileri eksik veya bozuk. Veri:",
            parsedData,
          );
          setDataError("Alınan sınav verileri eksik veya bozuk.");
          localStorage.removeItem(storageKeyUsed);
          console.log(
            `[RESULTS_PAGE_TRACE] localStorage.removeItem(${storageKeyUsed}) ÇAĞRILDI (bozuk veri).`,
          );
          dataLoadedRef.current = true;
          console.log(
            "[RESULTS_PAGE_STATE] Hata durumunda dataLoadedRef.current=true olarak ayarlandı.",
        ...
        const results = calculateResults(
          parsedQuizData.questions,
          parsedUserAnswersData,
          parsedQuizData.title || "Sınav Sonuçları",
        );
        console.log(
          "[RESULTS_PAGE_STATE] setQuizResult() ÇAĞRILIYOR. Sonuçlar:",
          results,
        );
        setQuizResult(results);
        console.log(
          "[RESULTS_PAGE_STATE] quizResult state güncellendi:",
          results,
        );
        console.log("[RESULTS_PAGE_TRACE] Veri başarıyla işlendi.");
      } catch (error) {
        console.error(
          "[RESULTS_PAGE_ERROR] Sınav sonuçları yüklenirken hata oluştu:",
          error,
          "| storageKeyUsed:",
          storageKeyUsed,
          "| storedDataString:",
        ...
        setDataError(
          `Sınav sonuçları yüklenirken bir hata oluştu: ${error instanceof Error ? error.message : String(error)}`,
        );
        if (storageKeyUsed && storedDataString) {
          localStorage.removeItem(storageKeyUsed);
          console.log(
            `[RESULTS_PAGE_TRACE] localStorage.removeItem(${storageKeyUsed}) ÇAĞRILDI (hata sonrası temizlik).`,
          );
        }
      } finally {
        console.log(
          `[RESULTS_PAGE_TRACE] FINALLY BLOĞU: loading state (önceki) =`,
          loading,
        );
        setLoading(false);
        console.log(
          "[RESULTS_PAGE_TRACE] setLoading(false) çağrıldı (finally bloğu).",
        );
        console.log(
          "[RESULTS_PAGE_TRACE] useEffect BİTTİ (finally bloğu sonrası). Son State'ler:",
          { loading, dataError, quizResult },
        );
      }
    })();
  ...
  useEffect(() => {
    ...
    localStorage.removeItem(storageKeyUsed);
    console.log(
      `[RESULTS_PAGE_TRACE] [CLEANUP_EFFECT] localStorage.removeItem(${storageKeyUsed}) ÇAĞRILDI (quizResult yüklendiği için).`,
    );
  ...
  if (dataError) {
    return (
      <div
        className={`min-h-screen flex flex-col justify-center items-center p-4 transition-all duration-300 ${
          isDarkMode
      ...
      <div className="relative z-10 p-4 sm:p-6">
        ...
        >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  >
                    ...
                    <p
                      className={`text-4xl font-bold mb-2 ${isDarkMode ? "text-green-400" : "text-green-600"}`}
                    >
                      {quizResult.questions?.filter((q) => q.isCorrect)
                        .length ?? 0}
                      <span
                        className={`text-2xl mx-1 ${isDarkMode ? "text-gray-500" : "text-gray-400"}`}
                      >
                        /
                      </span>
                      {quizResult.questions?.length ?? 0}
                  ...
                  >
                    ...
                    <p
                      className={`text-4xl font-bold mb-2 ${isDarkMode ? "text-purple-400" : "text-purple-600"}`}
                    >
                  ...
                  >
                    ...
                    <p
                      className={`text-4xl font-bold mb-2 ${isDarkMode ? "text-blue-400" : "text-blue-600"}`}
                    >
                      {
                        calculatedSubTopicStats.filter(
                          (stat) => stat.score >= 70,
                        ).length
                      }
                    </p>
                  ...
                  >
                    ...
                    >
                      <Brain
                        className={`w-8 h-8 ${isDarkMode ? "text-orange-400" : "text-orange-600"}`}
                      />
                    </div>
                    <p
                      className={`text-4xl font-bold mb-2 ${isDarkMode ? "text-orange-400" : "text-orange-600"}`}
                    >
                      {
                        calculatedSubTopicStats.filter(
                          (stat) => stat.score < 50,
                ...
                {calculatedSubTopicStats.map(
                  (
                    ...
                    >
                      ...
                      <div
                        className={`absolute left-0 top-0 bottom-0 w-2 rounded-l-xl transition-all duration-500 ${
                          stat.score >= 80
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center">
                          <div
                            className={`w-12 h-12 rounded-xl flex items-center justify-center mr-4 transition-all duration-300 group-hover:scale-110 ${
                              stat.score >= 70
                                ? isDarkMode
                                  ? "bg-green-500/20"
                                  : "bg-green-100"
                          ...
                          >
                            {stat.score >= 70 ? (
                              ...
                              <Clock
                                className={`w-6 h-6 ${isDarkMode ? "text-yellow-400" : "text-yellow-600"}`}
                              />
                            ) : (
                              <AlertCircle
                          ...
                          <div>
                            ...
                            >
                              {stat.correctQuestions} / {stat.totalQuestions}{" "}
                              doğru cevap
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="text-right">
                          <span
                            className={`text-3xl font-black ${getScoreColor(stat.score)}`}
                          >
                      ...
                      <div className="relative">
                        <div
                          className={`w-full rounded-full h-3 transition-all duration-300 ${
                        ...
                        <div
                          className={`absolute top-0 transition-all duration-1000 ease-out h-3 flex items-center`}
                          style={{ left: `${Math.min(stat.score, 95)}%` }}
                        >
                          <div
                            className={`w-2 h-2 rounded-full shadow-lg ${getProgressBarBgColor(stat.score)} -mt-0.5`}
                          ></div>
                        </div>
                      </div>
                        ...
                        <div className="flex items-center text-xs text-gray-500 dark:text-gray-400">
                          {stat.score >= 80 ? (
                            ...
                            <>
                              ...
                              İyi seviyedesiniz. Biraz daha çalışarak
          ...
          >
            {/* Section Decorations */}
            <div
              className={`absolute top-0 left-0 w-24 h-24 rounded-full opacity-10 blur-xl ${
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex items-start">
                      <div
                        className={`w-10 h-10 rounded-lg flex items-center justify-center mr-4 text-white font-bold text-lg shadow-lg ${
                          q.isCorrect
                            ? "bg-gradient-to-br from-green-500 to-emerald-400"
                            : "bg-gradient-to-br from-red-500 to-pink-400"
                        }`}
                      ...
                      <div className="flex-1">
                        <h3
                          className={`text-lg font-bold mb-2 ${isDarkMode ? "text-gray-100" : "text-gray-900"}`}
                        >
                          {q.questionText}
                        </h3>
                        {q.subTopic && (
                  ...
                  <motion.div
                    ...
                    className={`mb-6 p-4 rounded-xl border-l-4 ${
                      q.isCorrect
                        ? isDarkMode
                          ? "bg-green-500/10 border-l-green-400 border border-green-500/20"
                          : "bg-green-50 border-l-green-500 border border-green-200"
                        : isDarkMode
                          ? "bg-red-500/10 border-l-red-400 border border-red-500/20"
                  ...
                  >
                    <h4
                      className={`font-bold text-lg mb-4 ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}
                    >
                      📝 Seçenekler:
                    </h4>
                    <div className="grid gap-3">
                      {q.options.map((option, i) => {
                        const isUserAnswer =
                          normalize(option) === normalize(q.userAnswer);
                        ...
                        return (
                          <motion.div
                            ...
                            animate={{ opacity: 1, x: 0 }}
                            transition={{
                              delay: 2.2 + index * 0.1 + i * 0.05,
                              duration: 0.4,
                            }}
                            className={`flex items-center gap-4 p-4 rounded-xl transition-all duration-300 border-2 ${
                              isCorrectAnswer
                                ? isDarkMode
                                  ? "bg-green-500/20 border-green-500/40 shadow-green-500/20 shadow-lg"
                                  : "bg-green-100 border-green-300 shadow-green-200/50 shadow-lg"
                                : isUserAnswer && !isCorrectAnswer
                                  ? isDarkMode
                                    ? "bg-red-500/20 border-red-500/40 shadow-red-500/20 shadow-lg"
                                    : "bg-red-100 border-red-300 shadow-red-200/50 shadow-lg"
                                  : isDarkMode
